...
Class Action {
	...
	function save_contributions(){
		...
		$data .= ", email = '$email' ";
		$data .= ", tax_id = '$tax_id' ";
		$data .= ", total_amount_contributed = '$total_amount_contributed' ";
	...
	function delete_contributions(){
		extract($_POST);
		$delete = $this->db->query("UPDATE borrowers set total_amount_contributed= NULL where id = ".$id);
		if($delete)
			return 1;
	}
